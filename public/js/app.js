...
var projection = d3.geoConicConformal().parallels([43, 45.5]).rotate([120.5, 0]);
...
d3.json("/oregon-topo.json", function (error, oregon) {
  ...
  pathel = vis.append('path').datum(feature).attr('class', 'state').attr('d', path);

  function reproject(el) {
    var parent = d3.select(this.parentNode);
    var x = parent.select('.js-slider[data-extent="x0"]').node().value;
    var y = parent.select('.js-slider[data-extent="y0"]').node().value;

    projection.fitExtent([[x, y], [width, height]], feature);

    pathel.transition().attr('d', path);
  }
